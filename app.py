...
def setAverageList(newAverageList):
    averageList = newAverageList

def getAverageList():
    return averageList

def setTotalScore(newTotal):
    global totalScore
    totalScore = newTotal
...
def hello_world():
    ...
    if request.method=='POST':
        ...
        if form.validate():
            if(form.radiobuttons.data == 'user'):
                userHandles, tweetText = tweetPull.timelineScan(form.inputField.data)
                gotaverageList = getAverageList()
                return render_template('home.html', form=form, averageList=gotaverageList, userHandles=userHandles, tweetText=tweetText, averageScore=totalScore)
            elif(form.radiobuttons.data=='query'):
                userHandles, tweetText = tweetPull.searchQuery(form.inputField.data)
                gotaverageList = getAverageList()
                return render_template('home.html', form=form, averageList=gotaverageList, userHandles=userHandles,
                                       tweetText=tweetText, averageScore=totalScore)
        print(form.radiobuttons.data)
        print(form.inputField.data)
